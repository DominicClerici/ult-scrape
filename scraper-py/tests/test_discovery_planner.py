from app.discovery.facets import TAB_TYPE, FacetCatalog, FacetValue, SliceSpec
...
def test_initial_slices_one_per_genre_plus_untagged():
    ...
    untagged = [s for s in slices if s.filters == {"type": TAB_TYPE}]
    assert len(untagged) == 1
    assert all(s.depth == 0 for s in slices)
    assert TAB_TYPE == "Official"
...
def test_subdivide_adds_next_ladder_facet():
    parent = SliceSpec(filters={"type": TAB_TYPE, "genres": "4"}, depth=0)
...
def test_subdivide_descends_to_third_facet():
    parent = SliceSpec(filters={"type": TAB_TYPE, "genres": "4", "decade": "2020"}, depth=1)
    children = subdivide(parent, _catalog(), ladder=["genres", "decade", "tonality"])
...
def test_subdivide_returns_none_when_ladder_exhausted():
    parent = SliceSpec(
        filters={"type": TAB_TYPE, "genres": "4", "decade": "2020", "tonality": "1"}, depth=2
    )
    assert subdivide(parent, _catalog(), ladder=["genres", "decade", "tonality"]) is None


def test_sort_windows_one_per_sort():
    spec = SliceSpec(filters={"type": TAB_TYPE, "genres": "4"})
